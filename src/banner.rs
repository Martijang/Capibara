use figlet_rs::{FIGlet, Toilet, FIGure};
...
impl Fonts{
    ...
    pub fn new() -> Self{
        Self { standard: FIGlet::standard().unwrap() , slant: FIGlet::slant().unwrap(), future: Toilet::future().unwrap() }
    }
    ...
    pub fn future_font(&self) -> FIGure<'_>{
        self.future.convert("I am Capibara not Capybara :]").unwrap()
    }
...
impl BannerMaker{
    pub fn new() -> Self{
        Self { fonts: Fonts::new(), rng: rand::rng()}
    }
